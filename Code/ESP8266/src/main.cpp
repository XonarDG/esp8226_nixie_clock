...
DateTime GetNTPTime()
{
  ...
  DateTime time = DateTime(ntp_time.tm_year+1900, ntp_time.tm_mon+1, ntp_time.tm_mday, ntp_time.tm_hour, ntp_time.tm_min, ntp_time.tm_sec); //inserts tm struct into datetime class (rtc library takes only DateTime)
  return time;
}
...
void Multiplex(int hh, int h, int mm, int m, int ss, int s, int multiplex_delay_micro, int blank_delay_micro)
{
  ...
  SendData(shift_reg1, shift_reg_null, colon_arr[0]);   //send blanking data
...
void loop()
{
  ...
  //converts two digit time values into single digit values
